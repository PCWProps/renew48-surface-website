...
function pcw_membership_crm_enabled() {
    // Intentionally fail closed. A future activation requires consent, retention,
    // rate limiting, redaction, and an approved downstream-data contract.
    return false;
}
...
function pcw_membership_form($notice = '') {
    ...
    if (!pcw_membership_crm_enabled()) {
        return '<section class="pcw-membership-request" aria-labelledby="pcw-membership-request-title"><span class="pcw-kicker">Membership requests</span><h2 id="pcw-membership-request-title">This request path is not active on staging.</h2><p>Membership intake is disabled until consent, retention, rate limiting, logging redaction, and the downstream-data contract have been approved. Do not submit personal or health information here.</p>'.$notice_html.'<div class="pcw-actions">'.$jane.'</div></section>';
    }
...
add_action('rest_api_init', function () {
    register_rest_route('pcw/v1', '/membership-request', array(
        ...
        'callback' => function (WP_REST_Request $request) {
            if (!pcw_membership_crm_enabled()) {
                return new WP_Error('crm_acceptance_required', 'Membership requests are not active on this staging site.', array('status' => 503));
            }
